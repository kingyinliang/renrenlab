   ...
   <!--右侧公用模块-->
    ...
    <div class="scientificSearch" style="height: ;">
		<div id="thesis" style="margin-bottom: 25px;padding-bottom: 20px;">
			...
			<div id="list">
				<ul>
				 <c:forEach items="${result.re}" var="paperlist" begin="0" end="9"  varStatus="st">
                    	 <li>
                            <a href="${paperlist.url_s }" class="messageTit">${paperlist.title_cn_s }</a>
                            <p class="thesisText" title="${paperlist.keywords }">${paperlist.keywords }</p>
                            <p class="writer">${paperlist.authors_ims } | ${paperlist.year_if } | ${paperlist.workplace_ais }</p>
                            <a href="${paperlist.url_s }">${paperlist.url_s }</a>
                        </li>
                        </c:forEach>
					<!-- <li>
				...
				<div id="pages" style="text-align: center;overflow: hidden;">
						<a id="next" href="JavaScript:location.href='${rlab}/front/kejso/morePaper?pageNo=${pageNo +1}&pageSize=10&query='+encodeURI('${keyword}');">下一页</a>
					<c:if test="${pageNo != 0}">
						<a id="top" href="JavaScript:location.href='${rlab}/front/kejso/morePaper?pageNo=${pageNo -1}&pageSize=10&query='+encodeURI('${keyword}');">上一页</a>
					</c:if>
				</div>
			</div>
		</div>
...
<script src="${rlab}/front/js/common/main.js?_=20170905"></script>
