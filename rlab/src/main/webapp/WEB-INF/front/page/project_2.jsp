...
<div class="wrapper">
    ...
    <div class="scientificSearch" style="height: ;">
        <div id="project" style="margin-bottom: 25px;padding-bottom: 20px;">
            ...
            <div class="list">
                <ul>
                    <c:forEach items="${relatedProject}" var="prolist" begin="0" end="9" varStatus="st">
                        <li>
                            <a  class="projectTit" title="${prolist.title}" style="cursor: pointer"><p>${prolist.title}</p></a>
                            <c:if test="${prolist.organization!=null && fn:length(prolist.organization)>0}">
                                <p >负责单位：<span style="color: #4E4E4E;"><c:forEach items="${prolist.organization}" var="org" >${org eq "null"?"不详":org} </c:forEach></span></p>
                            </c:if>
                            <c:if test="${prolist.organization==null && fn:length(prolist.organization)<=0}">
                                <p >负责单位：<span style="color: #4E4E4E;">不详</span></p>
                            </c:if>
                            <c:if test="${prolist.leader!=null && fn:length(prolist.leader)>0}">
                                <p class="rightMark"><span class="mark" style="width:32px;">${prolist.year}</span> <span style="float:right;margin: 0 6px;">|</span> <span title="${prolist.leader}" style="color: #4E4E4E;float: right;"><c:forEach items="${prolist.leader}" var="leader" >${leader eq "null"?"不详":leader} </c:forEach></span></p>
                            </c:if>
                            <c:if test="${prolist.leader==null && fn:length(prolist.leader)<=0}">
                                <p class="rightMark"><span class="mark" style="width:32px;">${prolist.year}</span> <span style="float:right;margin: 0 6px;">|</span> <span title="${prolist.leader}" style="color: #4E4E4E;float: right;">不详</span></p>
                            </c:if>
                        </li>
                    </c:forEach>
                    <!-- <li>
                ...
                <div id="pages" style="text-align: center;overflow: hidden;">
                    <c:if test="${relatedProject!=null && fn:length(relatedProject)>9}">
                    <%--<c:if test="${relatedProject!=null}">--%>
                        <a id="next" href="JavaScript:location.href='${rlab}/front/superSearch/morePro?flag=${flag}&pageNo=${pageNo +1}&pageSize=10&query='+encodeURI('${keyword}');">下一页</a>
                    </c:if>
                    <c:if test="${pageNo != 0}">
                        <a id="top" href="JavaScript:location.href='${rlab}/front/superSearch/morePro?flag=${flag}&pageNo=${pageNo -1}&pageSize=10&query='+encodeURI('${keyword}');">上一页</a>
                    </c:if>
                </div>
            </div>
        </div>
