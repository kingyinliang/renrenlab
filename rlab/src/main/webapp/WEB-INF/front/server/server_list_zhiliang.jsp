...
<div class="wrapper">
    ...
    <jsp:include page="../template/header.jsp" flush="true">
        <jsp:param name="selected" value="6"/>
    </jsp:include>
    <%-- START高级搜索--%>
    ...
    <div id="listContent" class="list_wrapper">
        <c:forEach items="${infos.list}" var="item">
            <div class="item clearfix">
                ...
                <div class="itemText">
                    <p class="itemTit" onclick="goserver(this)" data-map-id="${item.serviceId}"><a
                            title="${item.serviceName}">${item.serviceName.length() > 30 ? item.serviceName.substring(0,29).concat('...') :item.serviceName}</a>
                    </p>
                    ...
                    <p>服务领域<span><c:forEach items="${item.serviceScope}" var="scope">${scope}</c:forEach></span></p>
...
<script type="text/javascript">
    ...
    var flags='zhiliang';
    var fenyes='/service/zhiliang?classify=${classify}&name=${name}';
    // 初始化页面分页
...
//        var URL = BASE_URL + "/service/search?keyword=" + encodeURI(keywords) + "&pageNo=1&pageSize=10";
    ...
    function goserver($this) {
        $this = $($this);
        window.location.href = "${rlab}/service/zhiliang/detail/" + $this.data("mapId");
    }
</script>
</body>
</html>
