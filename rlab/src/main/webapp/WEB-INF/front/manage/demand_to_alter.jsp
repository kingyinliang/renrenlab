...
<div class="wrapper" style="background-color: #f3f6f9">
            ...
            <div class="inputBG radius ">
                ...
                <div class="txt_box">
                    ...
                    <textarea maxlength="140" class="input"
                              placeholder="为了保证您获得质量更佳的服务，请具体描述该实验项目的可公开信息，包括但不限于研究目的/背景介绍、实验样品（名称、数量、状态、处理方式等）、实验方法（标准、技术规范等）、实验仪器设备（名称、品牌、型号等）以及服务方资质要求等。">${detail.uReqDescription}</textarea>
                </div>
                <button class="publish" type="button"
                        style="">${Integer.valueOf(detail.remainTime) <= 0? '重新发布': '修改需求'}</button>
            </div>
        </div>
    ...
    <div id="modify" style="width: 400px; height: 170px; background-color: #cccccc; display: none">
        <p style="font-size: 16px; text-align: center; margin-top: 20px">你是否确认修改需求？</p>
        <button style="margin-left: 70px; float: left; background-color: white; border: 1px solid #ccc; color: #999" class="dismiss button">取消</button>
        <button style="margin-right: 70px; float: right; background-color: #508df0; color: white" class="commit button">确定</button>
    </div>
</div>
</body>
...
<script type="text/javascript">
    ...
    $(".publish").on("click", function () {
        ...
        $(".commit").off("click").on("click", function () {
            ...
            $.ajax({
                url: '${rlab}/front/user/req/modify',
                type: 'POST',
                dataType: "json",
