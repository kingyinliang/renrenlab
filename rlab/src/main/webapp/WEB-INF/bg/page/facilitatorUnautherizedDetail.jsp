        ...
        <div class="main" style="overflow-y: auto">
            <div class="hdNav">
                <span class="layui-breadcrumb">
                    <a><cite>服务商管理</cite></a>
                    <a href="${rlab}/page/facilitator/un">未认证服务商列表</a>
                    <a><cite>详情</cite></a>
                </span>
            </div>
            <div class="infModify">
                <div class="orgCorporation">
                    <div class="clearfix">
                        ...
                        <p class="business">
                            ...
                            <span>审核状态：
                                <c:choose>
                                    <c:when test="${orgInfo.orgIdentification eq 3}">
                                        <s>${orgInfo.orgIdentificationStr}<i class="lab-gantanhao" onclick="showre()"></i></s>
                                    </c:when>
                                    <c:otherwise>
                                        ${orgInfo.orgIdentificationStr}
                                    </c:otherwise>
                                </c:choose>
                            </span>
...
<script>
    ...
    function gotan() {
        var html='<div class="tan" style="padding: 20px">\
            ...
            <p style="text-align: center;font-size: 16px;color: #333;line-height: 50px">审核结果</p>\
            <textarea placeholder="请输入拒绝原因" style="width: 485px;height: 200px" id="reason"></textarea>\
            <div style="text-align: center;margin-top: 10px">\
            <button class="layui-btn" style="background-color:#7b8da0" id="revovation">拒绝</button>\
            <button class="layui-btn" id="revovations">通过</button>\
            </div>\
            </div>';
        //页面层-自定义
        ...
        $("#revovations").off("click").on("click", function () {
            revocation(2);
        })
        $("#revovation").off("click").on("click", function () {
            revocation(3);
        })
    }
    function revocation(num) {
        ...
        $.ajax({
            url: '${rlab}/bg/provider/audit?oid=${orgInfo.orgOid}&state='+num+'&reason='+reason,
            type: 'GET',
...
//            window.location.reload(true);
    ...
    function showre() {
        var html='<div class="tan" style="padding: 20px">\
            <span class="lab-close-1" style="font-size: 22px;position: absolute;right: 10px;top: 10px;cursor: pointer" id="closetan"></span>\
            <p style="text-align: center;font-size: 16px;color: #333;line-height: 50px">拒绝原因</p>\
            <p style="color: red;text-align: center">${orgInfo.rejectReason}</p>\
            <div style="text-align: center;margin-top: 10px">\
                <button class="layui-btn" id="closetans">确定</button>\
            </div>\
            </div>';
        //页面层-自定义
        ...
        $("#closetans").off("click").on("click", function () {
            layer.close(layer_2);
        })
    }
