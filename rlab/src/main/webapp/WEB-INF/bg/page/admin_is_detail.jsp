...
<head>
    ...
    <style type="text/css">
        ...
        .admin_info label .ipt_min input.texts {
            background: #f2f5f8;
            color: #4c5e70;
        }
        ...
        <div class="main" style="overflow-y: auto">
            ...
            <div class="hdNav">
                <span class="layui-breadcrumb">
                    ...
                    <a href="${rlab}/bg/user/role_manage">管理员列表</a>
                    <a href="#">详情</a>
                </span>
            </div>
                ...
                <form class="layui-form" action="">
                    ...
                    <div class="admin_info clearfix">
                        <label class="form_coomon" style="width: 325px;">
                            <s>邮&nbsp;&nbsp;&nbsp;&nbsp;箱：</s>
                            <div class="ipt_min">
                                <input id="email" type="text" class="texts" placeholder="请输入邮箱" value="${detail.uEmail}"
                                       disabled>
                            </div>
                        </label>
                        <label class="form_coomon" style="width: 325px; margin-left: 68px;">
                            <s>初始密码：</s>
                            <div class="ipt_min">
                                <input id="pwds" onblur="hidePwdIpt()" type="text" class="texts" placeholder="输入初始密码" value="******"
                                       disabled>
                            </div>
                            <i id="iptBtn" class="lab-alter" onclick="showPwdIpt(this)"></i>
                        </label>
                        <label class="form_coomon" style="width: 325px;">
                            <s>个人姓名：</s>
                            <div class="ipt_min">
                                <input id="name" type="text" placeholder="输入员工姓名" class="texts" value="${detail.uNickname}" disabled>
                            </div>
                        </label>
                        <label class="form_coomon" style="width: 325px;margin-left: 68px;">
                            <s>联系电话：</s>
                            <div class="ipt_min">
                                <input id="phone" type="text" placeholder="输入联系电话" class="texts" value="${detail.uMobile}" disabled>
                            </div>
                        </label>
                        <label class="form_coomon" style="width: 325px;">
                            <s>工&nbsp;&nbsp;&nbsp;号：</s>
                            <div class="ipt_min">
                                <input id="number" type="text" placeholder="输入输入员工编号" class="texts" value="${detail.uJobNumber}" disabled>
                            </div>
                        </label>
                        <label class="form_coomon" style="width: 325px;margin-left: 68px;">
                            <s>部&nbsp;&nbsp;&nbsp;门：</s>
                            <div class="ipt_min">
                                <select lay-filter="changeDepartment">
                                    <%--<option value="">请选择...</option>--%>
                                    <option value="总裁办" ${detail.uDepartment=='总裁办'?'selected':''}>总裁办</option>
                                    <option value="运营部" ${detail.uDepartment=='运营部'?'selected':''}>运营部</option>
                                    <option value="产品部" ${detail.uDepartment=='产品部'?'selected':''}>产品部</option>
                                    <option value="市场部" ${detail.uDepartment=='市场部'?'selected':''}>市场部</option>
                                    <option value="技术部" ${detail.uDepartment=='技术部'?'selected':''}>技术部</option>
                                    <option value="客服部" ${detail.uDepartment=='客服部'?'selected':''}>客服部</option>
                                </select>
                            </div>
                        </label>
                    </div>
                            ...
                            <div class="ipt_min" style="">
                                <select id="changeAdmin" lay-filter="changeAdmin">
                                    <c:if test="${'超级管理员'.equals(sessionScope.u_r_name)}">
                                        <option value="10008" ${detail.rName=='高级管理员'?'selected':''}>高级管理员</option>
                                    </c:if>
                                    <option value="10006" ${detail.rName=='中级管理员'?'selected':''}>中级管理员</option>
                                    <option value="10005" ${detail.rName=='初级管理员'?'selected':''}>初级管理员</option>
                                </select>
                            </div>
                        </label>
...
<script type="text/javascript">
    ...
    var department='${detail.uDepartment}';
    var admin = '${detail.rRid}';
    function showPwdIpt($_this) {
        $("#pwds").removeAttr("disabled").css({background: "#fff", color: "#2c415a"});
        $("#iptBtn").hide();
    }

    function hidePwdIpt() {
        $("#pwds").prop("disabled", true).css({background: "#f2f5f8", color: "#4c5e70"});
        $("#iptBtn").show();
