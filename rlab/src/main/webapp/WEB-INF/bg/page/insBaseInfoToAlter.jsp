        ...
        <div class="main" style="overflow-y: auto">
            <div class="hdNav">
                <span class="layui-breadcrumb">
                    ...
                    <a><cite>基本信息修改</cite></a>
                </span>
            </div>
            <div class="infModify">
                <p>仪器名称<em>*</em>：<input type="text" class="insName" placeholder="" value="${baseInfo.insName}" id="mname" maxlength="100"></p>
                <p>仪器型号<em>*</em>：<input type="text" class="insType" placeholder="" value="${baseInfo.insMode}" id="mtype" maxlength="100"></p>
                <div class="clearfix infP">
                    <div style="float: left;line-height: 7">仪器图片<em>*</em>：</div>
                    ...
                    <div class="addImageBtn" id="addImageBtn">
                        <a class="insImg file"><img src="${rlab}/bg/img/add_status_default.png" alt=""></a>
                        <input id="addImageInput" type="file" name="file">
                        <span style="float: left" id="numss">(0/8)</span>
                    </div>
                </div>
                <p>仪器品牌<em>*</em>：<input type="text" class="insBrand" placeholder="" value="${baseInfo.insBrand}" id="insBrand" maxlength="100"></p>
                <p>仪器产地<em>*</em>：<input type="text" class="insAddress" placeholder="" value="${baseInfo.insOrigin}" id="insOrigin" maxlength="100">
                </p>
                <div class="clearfix infP">
                    ...
                    <div class="listNature" id="instype2">
                        <div class="input layui-form-select"><span data-cate-id="${baseInfo.insturmentCategories[1].insCategory}">${baseInfo.insturmentCategories[1].categoryName}</span><i class="layui-edge"></i></div>
                        <dl class="dldd">
                            <dd data-cate-id="${baseInfo.insturmentCategories[1].insCategory}">${baseInfo.insturmentCategories[1].categoryName}</dd>
                        </dl>
                    </div>
                    <div class="listNature" id="instype3">
                        <div class="input layui-form-select"><span data-cate-id="${baseInfo.insturmentCategories[2].insCategory}">${baseInfo.insturmentCategories[2].categoryName}</span><i class="layui-edge"></i></div>
                        <dl class="dldd">
                            <dd data-cate-id="${baseInfo.insturmentCategories[2].insCategory}">${baseInfo.insturmentCategories[2].categoryName}</dd>
                        </dl>
                     </div>
                </div>
                    ...
                    <div class="infTable">
                        <table class="layui-table">
                            ...
                            <c:forEach items="${coreParamList}" var="core">
                                <tr>
                                    <td class="infArg"><input type="text" placeholder="" value="${core.title}" maxlength="100"></td>
                                    <td class="infContent"><input type="text" placeholder="" value="${core.content}"
                                                                  maxlength="100"></td>
                                    <td style="text-align: center;color: #6693c8"><span class="lab-del" style="cursor: pointer" onclick="removetr(this)"></span></td>
                                </tr>
                            </c:forEach>
                            </tbody>
                        </table>
                    </div>
                    <span class="lab-addicon add" onclick="addtr()"></span>
                </div>
                <p class="addDes">仪器简介<em>*</em>：<button class="layui-btn btn" id="addDes"><div class="lab-qr-code-1"></div>添加描述</button></p>
                <div class="addDesap">
                    <c:forEach items="${descriptionList}" var="des">
                        <div class="addDesmain">
                            <div class="clearfix"><a onclick="removeDes(this)"><label class="lab-del"></label>删除</a><a
                                    onclick="modifyDes(this)"><label class="lab-alter"></label>编辑</a></div>
                            <div class="desMain">
                ...
                <div class="save">
                    <button class="layui-btn" onclick="save()" data-ins-id="${baseInfo.insIid}" id="save">保存仪器</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="shade" id="shade">
        <div class="tit">
            ...
            <input type="text" name="" value="" id="tit" placeholder="请输入..."
                   style="margin-top: 10px; border-color: #666; border-width: 1px;" maxlength="100"/>
            <p style="margin: 10px 0px">描述正文</p>
            <textarea name="" rows="" cols="" id="describe" placeholder="请输入..."
                      style="border-color: #666; border-width: 1px;" maxlength="5000"></textarea>
...
<script>
    ...
    var instype1 = "${baseInfo.insturmentCategories[0].insCategory}",
        instype2 = "${baseInfo.insturmentCategories[1].insCategory}",
        instype3 = "${baseInfo.insturmentCategories[2].insCategory}";
    var instypeValue = '${baseInfo.insturmentCategories[0].categoryName}';

    function addtr() {
        var trs=$('<tr>\
            <td class="infArg"><input type="text" placeholder="" value="" maxlength="100"></td>\
            <td class="infContent"><input type="text" placeholder="" value="" maxlength="100"></td>\
            <td style="text-align: center;color: #6693c8"><span class="lab-del" style="cursor: pointer" onclick="removetr(this)"></span></td>\
            </tr>');
        $(".infTable tbody").append(trs);
    }
...
//            console.log(e);
    ...
    function save() {
        ...
        //核心参数
        for (var i = 0; i < $(".infTable tr").length; i++) {
            var key=$(".infTable tr").eq(i).find(".infArg input").val();
            insCoreParam[key] =$(".infTable tr").eq(i).find(".infContent input").val();
        }
        //图片
        ...
        if (name.length == 0) {
            alert("仪器名称不能为空");
            return;
        } else if (insBrand.length == 0) {
            alert("仪器品牌不能为空");
            return;
        } else if (type.length == 0) {
            alert("仪器型号不能为空");
            return;
        } else if (insOrigin.length == 0) {
            alert("仪器产地不能为空");
            return;
        } else if (insCoreParam.length == 0) {
            alert("核心参数不能为空");
            return;
        ...
        var insCoreParams = JSON.stringify(insCoreParam);
        ...
        var data = {
            ...
            insName: name,
            insPic: imgSrcParam,
            insCategory: insCategorys,
            insCoreParam:insCoreParams
        };
