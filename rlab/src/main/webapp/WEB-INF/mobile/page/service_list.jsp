...
<div id="main" class="wrapper" style="display: block">
    ...
    <div class="goods_tab">
        ...
        <span onclick="choseServiceList()" class="cur">服务<i>&nbsp;(${infos.total})</i></span>
        ...
        <div class="funny_list">
            ...
            <ul>
                <c:forEach items="${interests}" var="item" varStatus="status">
                    <li onclick="toSerachService(this)">${item}</li>
                </c:forEach>
                <a href="javascript:void(0);" onclick="toSuperSearch('service')">
    ...
    <div class="goods_list">
        ...
        <c:if test="${other != 4}">
            <c:forEach items="${infos.list}" var="item">
                <div class="item" data-service-id="${item.serviceId}" onclick="toServicesDetail(this)">
                    <div class="lft_img">
                    ...
                    <div class="rt_info">
                        <p class="int_name">
                            <c:out value="${ item.serviceName.length() > 13 ? item.serviceName.substring(0,10).concat('...') :item.serviceName }"
                                   escapeXml="true"/>
                        </p>
                            <%--<p class="int_type">型号：--%>
                        ...
                        <p class="int_about">
                            <span class="ad">
                                <%--<c:out value="${item.serviceProvince}" escapeXml="true"/>--%>
                                   ...
                                   <c:choose>
                                       ...
                                       <c:when test="${item.servicePrice.flag == 2}">
                                           <c:if test='${item.servicePrice.scopeHighPrice.equals("0")}'>
                                               ...
                                               <c:if test='${!"无".equals(item.servicePrice.unit)}'>
                                                   ￥${item.servicePrice.scopeLowPrice}<i>/${item.servicePrice.unit.length() > 3? item.servicePrice.unit.substring(0,2).concat('...') :item.servicePrice.unit}</i>起
                                               </c:if>
                                           </c:if>
                                           ...
                                           <c:if test='${!item.servicePrice.scopeHighPrice.equals("0")}'>
                                               ...
                                               <c:if test='${!"无".equals(item.servicePrice.unit)}'>
                                                   ￥${item.servicePrice.scopeLowPrice} - ${item.servicePrice.scopeHighPrice}
                                                   <i>/${item.servicePrice.unit.length() > 3? item.servicePrice.unit.substring(0,2).concat('...') :item.servicePrice.unit}</i>
                                               </c:if>
                                           </c:if>
                                       </c:when>
...
<script>
     ...
     */
    function initPaging() {
        showPages(${infos.total}, (${pageNo} -1) * 10, 10, function (from, max) {

            dealParams();
            // 处理页码参数
            PAGE_NO = from / 10 + 1;
            toServiceList();// 跳转搜索结果页
...
//        $("#div_moveToTop").show();
    ...
    function toServicesDetail($this) {
        location.href = BASE_URL + "/service/detail/" + $($this).data("serviceId");
    }

    /**
     * 搜索服务
     */
    function toSerachService($this) {
