...
<div id="main" class="wrapper" style="display: block">
    ...
    <%--END 头部公用部分引入--%>
    <div class="goods_tab">
        <span onclick="choseInsList()" class="">仪器<i>&nbsp;(${insSize})</i></span>
        <span onclick="choseServiceList()" class="">服务<i>&nbsp;(${serviceSize})</i></span>
        <span onclick="choseOrgList()" class="cur">机构<i>&nbsp;(${total})</i></span>
    </div>
        ...
        <div class="funny_list">
            ...
            <ul>
                <c:forEach items="${interestArr}" var="item" varStatus="status">
                    <li onclick="toOrgListByKeso(this)">${item}</li>
                </c:forEach>
                <a href="javascript:void(0);" onclick="toSuperSearch('org')">
    ...
    <div class="goods_list">
        ...
        <c:if test="${other != 4}">
            ...
            <c:forEach items="${infos.list}" var="item">
                <div class="item" data-org-id="${item.orgOid}" onclick="toOrgDetail(this)">
                    <div class="lft_img">
                        ...
                        <c:if test="${item.orgType==2}">
                            <i class="renzheng lab-renzheng_1"></i>
                        </c:if>
                    </div>
                    <div class="rt_info">
                        <p class="int_name">
                        ...
                        <p class="int_about">
                            <span class="ad">${item.orgAddrCity}${item.orgAddrDistrict}</span>
                        </p>
                    </div>
                </div>
        ...
        <%--<c:if test="${other != 4}">--%>
        <%--<c:forEach items="${infos.list}" var="item">--%>
        <%--<div class="item" data-goods-id="${item.serviceId}" onclick="toGoodsDetail(this)">--%>
        <%--<div class="lft_img">--%>
        <%--<c:if test="${item.servicePic == null}">--%>
        <%--<img src="${rlab}/common/common_img/default/ins_300X300.jpg" alt="商品图片">--%>
        <%--</c:if>--%>
        <%--<c:if test="${item.servicePic != null}">--%>
        <%--&lt;%&ndash;该仪器图片存在时&ndash;%&gt;--%>
        <%--<img onerror="imgOnError(this)" src="${item.servicePic}300_300" alt="商品图片">--%>
        <%--</c:if>--%>
        <%--</div>--%>

        <%--<div class="rt_info">--%>
        <%--<p class="int_name">--%>
        <%--<c:out value="${ item.serviceName.length() > 15 ? item.serviceName.substring(0,14).concat('...') :item.serviceName }"--%>
        <%--escapeXml="true"/>--%>
        <%--</p>--%>
        <%--&lt;%&ndash;<p class="int_type">型号：&ndash;%&gt;--%>
        <%--&lt;%&ndash;<c:out value="${ item.insMode.length() > 15 ? item.insMode.substring(0,15).concat('...') :item.insMode }" escapeXml="true"/>&ndash;%&gt;--%>
        <%--&lt;%&ndash;</p>&ndash;%&gt;--%>
        <%--<p class="int_about">--%>
        <%--<span class="ad">--%>
        <%--&lt;%&ndash;<c:out value="${item.serviceProvince}" escapeXml="true"/>&ndash;%&gt;--%>
        <%--<c:out value="${item.serviceCity}" escapeXml="true"/>--%>
        <%--<c:out value="${item.serviceDistrict}" escapeXml="true"/>--%>
        <%--</span>--%>
        ...
        <%--￥${item.servicePrice.accPrice}--%>
        <%--</c:if>--%>
        <%--<c:if test='${!"无".equals(item.servicePrice.unit)}'>--%>
        <%--￥${item.servicePrice.accPrice}<i>/${item.servicePrice.unit}</i>--%>
        <%--</c:if>--%>
        <%--</c:when>--%>
        ...
        <%--</c:if>--%>
        <%--<c:if test='${!"无".equals(item.servicePrice.unit)}'>--%>
        <%--￥${item.servicePrice.scopeLowPrice}<i>/${item.servicePrice.unit}</i>起--%>
        <%--</c:if>--%>
        <%--</c:if>--%>

        <%--<c:if test='${!item.servicePrice.scopeHighPrice.equals("0")}'>--%>
        <%--<c:if test='${"无".equals(item.servicePrice.unit)}'>--%>
        <%--￥${item.servicePrice.scopeLowPrice} - ${item.servicePrice.scopeHighPrice}--%>
        <%--</c:if>--%>
        <%--<c:if test='${!"无".equals(item.servicePrice.unit)}'>--%>
        <%--￥${item.servicePrice.scopeLowPrice} - ${item.servicePrice.scopeHighPrice}--%>
        <%--<i>/${item.servicePrice.unit}</i>--%>
        <%--</c:if>--%>
        <%--</c:if>--%>
        <%--</c:when>--%>
...
//        $("#div_moveToTop").show();
    ...
    function toOrgDetail($this) {
        location.href = BASE_URL + "/org/" + $($this).data("orgId");
    }
