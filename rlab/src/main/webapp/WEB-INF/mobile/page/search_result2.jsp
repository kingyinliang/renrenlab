...
<div id="main" class="main" style="background: white;">
    ...
    <div class="fixedTab" style="display: ${!empty all ?'none':'block'}">
        ...
        <a href="${rlab}/front/search/p2?searchKey=${keyword}" class="seledTab">科研众包</a>
    ...
    <div class="typebox clearfix" style="top: ${!empty all ?'2.245rem':'3.345rem'};">
        <div class="stairType">
            <i class="lab-top"></i>
            <c:forEach items="${category}" var="item" varStatus="in">
                <c:if test="${in.index==0}">
                    <div><p data-category-id="99999" onclick="second(this )" class="seledType">全部分类</p></div>
                    <div><p data-category-id="${item.id}">${item.classifyName}</p></div>
                </c:if>
                <c:if test="${in.index!=0}">
                    <div><p data-category-id="${item.id}">${item.classifyName}</p></div>
                </c:if>
...
<script type="text/javascript">
    ...
    $("#listmain ul").append(getlist(ids, 1, $(".city span").text()));//初始化列表
    /**
     * 初始化地址下拉列表
     */
    ...
    function choseAddress($this) {
        ...
        $("#listmain ul").html("").data("typeStr", ids).data('pageNo', 2).data("cityStr", curAddress);
        $("#listmain ul").append(getlist(ids, 1, curAddress));
    ...
    function second($this) {
        ...
        $("#listmain ul").html("").data("typeStr", ids).data('pageNo', 2);
        $("#listmain ul").append(getlist(ids, 1, city));
        mui('#listmain').pullRefresh().scrollTo(0, 0, 10);
    }
    ...
    function gettypelist(num) {
        var html = '';
        $.ajax({
            url: BASE_URL + '/service/classify?id=' + num,
            type: 'GET',
            dataType: "json",
            ...
            success: function (data) {
                ...
                for (var i = 0; i < data.payload.length; i++) {
                    html += '<div><p data-category-id="' + data.payload[i].classifyMap + '" onclick="second(this)">' + data.payload[i].classifyName + '</p></div>'
                }
    ...
    function getlist(str,num,city,ul) {
        ...
        var li;
        $.ajax({
            url: BASE_URL + '/front/search/cro?tag=科研众包&keyword=${keyword}&pageNo=' + num + '&category2=' + str + '&city=' +city,
            type: 'GET',
            dataType: "json",
            contentType: "application/json",
            async: false,
