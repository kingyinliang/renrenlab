...
<div id="main" class="org_list">
    ...
    <%--END 头部样式引入（中间带logo）--%>
    <div class="totals">
        共：<span>${total}</span>条服务项目
    </div>
    <%--仪器列表开始--%>
    <div class="goods_list">
        <c:forEach items="${infos.list}" var="info">
            <div class="item" data-goods-id="${info.serviceId}" onclick="toserviceDetail(this)">
                <div class="lft_img">
                    <c:if test="${info.servicePic == null}">
                        <img src="${rlab}/common/common_img/default/ins_300X300.jpg" alt="商品图片">
                    </c:if>
                    <c:if test="${info.servicePic != null}">
                        <%--该仪器图片存在时--%>
                        <img onerror="imgOnError(this)" src="${info.servicePic}" alt="商品图片">
                    </c:if>
                </div>
                ...
                <div class="rt_info">
                        <%--仪器名称--%>
                    <p class="int_name">
                        <c:out value="${info.serviceName.length() > 15 ? info.serviceName.substring(0,14).concat('...') :info.serviceName }"
                               escapeXml="true"/>
                    </p>
                    <p class="int_about">
                            <%--仪器地址--%>
                        <span class="ad">
                            <c:out value="${info.serviceProvince}" escapeXml="true"/><c:out value="${info.serviceCity}"
                                                                                            escapeXml="true"/>
                        </span>
                            <%--仪器价格--%>
                        <span class="price">
                           <c:choose>
                               <c:when test="${info.servicePrice.flag == 1}">
                                   <c:if test='${"无".equals(info.servicePrice.unit)}'>
                                       ￥${info.servicePrice.accPrice}
                                   </c:if>
                                   <c:if test='${!"无".equals(info.servicePrice.unit)}'>
                                       ￥${info.servicePrice.accPrice}<i>/${info.servicePrice.unit}</i>
                                   </c:if>
                               </c:when>
                               <c:when test="${info.servicePrice.flag == 2}">
                                   <c:if test='${info.servicePrice.scopeHighPrice.equals("0")}'>
                                       <c:if test='${"无".equals(info.servicePrice.unit)}'>
                                           ￥${info.servicePrice.scopeLowPrice}起
                                       </c:if>
                                       <c:if test='${!"无".equals(info.servicePrice.unit)}'>
                                           ￥${info.servicePrice.scopeLowPrice}<i>/${info.servicePrice.unit}</i>起
                                       </c:if>
                                   </c:if>

                                   <c:if test='${!info.servicePrice.scopeHighPrice.equals("0")}'>
                                       <c:if test='${"无".equals(info.servicePrice.unit)}'>
                                           ￥${info.servicePrice.scopeLowPrice} - ${info.servicePrice.scopeHighPrice}
                                       </c:if>
                                       <c:if test='${!"无".equals(info.servicePrice.unit)}'>
                                           ￥${info.servicePrice.scopeLowPrice} - ${info.servicePrice.scopeHighPrice}
                                           <i>/${info.servicePrice.unit}</i>
                                       </c:if>
                                   </c:if>
                               </c:when>
                               <c:when test="${info.servicePrice.flag == 3}">
                                   面议
                               </c:when>
                           </c:choose>
                         </span>
...
<script type="text/javascript">
    ...
    showPages('${total}', ('${pageNo}' - 1) * 10, 10, function (from, max) {
            backFlag++;
            PAGE_NO = from / 10 + 1;
        window.location.href = BASE_URL + '/org/${oid}/service?pageNo=' + PAGE_NO + '&pageSize=10&backFlag=' + backFlag;
    ...
    function toserviceDetail($this) {
        window.location.href = BASE_URL + '/service/detail/' + $($this).data("goodsId");
    }
