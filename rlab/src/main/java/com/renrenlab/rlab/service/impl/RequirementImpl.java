...
public class RequirementImpl implements RequirementService {
    ...
    public int delay(Long uReqId) {
        ...
        if (Integer.valueOf(SearchUtil.getRemainTime(requirement.getuReqEndTime())) > 3) {
            return 0;
        }
        ...
        if (requirement.getuReqIds() != null) {
            for (Long id : requirement.getuReqIds()) {
                ...
                if (result > 0 && (requirement.getuReqState() == 1 || requirement.getuReqState() == 4)) {
                    SMSUtils.sendMessage(requirement1.getuMobile(), SmsConstant.REQ_STATE_MODIFY, null);
                }
            }
        }
        return result;
        ...
        if (requirements != null) {
            for (Requirement requirement : requirements) {
                ...
                if (requirement.getuHasDelay() == 1) {
                    //已经延期一次
                    SMSUtils.sendMessage(requirement.getuMobile(), SmsConstant.WILLBE_OUT_DATE2);
                } else {
                    //没有延期过
                    SMSUtils.sendMessage(requirement.getuMobile(), SmsConstant.WILLBE_OUT_DATE);
                }
    ...
    @Override
    public PageInfo<Requirement> getListByUid(Long uid, Integer state, Integer pageNo, Integer pageSize) {
        PageHelper.startPage(pageNo, pageSize);
    ...
    public Requirement getDetailById(Long uReqId) {
        Requirement requirement = requirementDao.selectByUreqId(uReqId);
        if (requirement != null) {
            requirementDao.addCount(requirement.getuReqId());
        }
        return requirement;
    }
    ...
    @Override
    public List<Requirement> mGetTinyList() {
        //未解决的需求
        List<Requirement> list = requirementDao.getNewRequirement();
        //未解决的需求会过期（状态为4的）
        handleReqs(list);
        //已解决的需求
        ...
        if (list.size() < 5) {
            //已解决的需求不会过期（状态为3的）
            list2 = requirementDao.getNewRequirement2();
        }
        if (list2 != null) {
            list.addAll(list2);
        }
    ...
    private void handleReqs(List<Requirement> requirements) {
