...
import com.renrenlab.rlab.vo.KejsoEchartsNodesInfo;
 ...
 */
@Controller
@RequestMapping(value = "/front/instrument")
public class FrontInsController {
    ...
    public String search(HttpServletRequest request,
        ...
        //session 中没有经纬度
        if (request.getSession().getAttribute("lng") == null) handleLocation(request);
        PageInfo<InsListInfo> infos = frontInsService.searchInstrument(keyword, insServiceType, scopeLowPrice, scopeHighPrice,
                category1, category2, province, city, district, pageNo, pageSize, false, request);
        //返回仪器分类
        ...
        if (infos.getTotal() == 0) {
            ...
            infos = frontInsService.searchInstrument(keyword, insServiceType, scopeLowPrice, scopeHighPrice,
                    category1, category2, province, city, null, pageNo, pageSize, true, request);
            other = 1;
        }
        if (infos.getTotal() == 0) {
            //该城市没有仪器 city,district置空
            infos = frontInsService.searchInstrument(keyword, insServiceType, scopeLowPrice, scopeHighPrice,
                    category1, category2, province, null, null, pageNo, pageSize, true, request);
            other = 2;
        }
        ...
        if (infos.getTotal() == 0) {
            ...
            infos = frontInsService.searchInstrument(keyword, insServiceType, scopeLowPrice, scopeHighPrice,
                    category1, category2, null, null, null, pageNo, pageSize, true, request);
            other = 3;
        }
        if (infos.getTotal() == 0) {
            other = 4;
        ...
        if (StringUtils.isNotEmpty(keyword)) {
            interestArr = new JSONArray();
            List<Map<String, Object>> interets = (List<Map<String, Object>>) superSearchService.searchInterestWords(keyword);
            if (interets != null) {
                interestArr.clear();
                for (Map<String, Object> map : interets) {
                    interestArr.add(map.get("text"));
                }
            } else {
                //没有感兴趣的领域，检索相关领域
                KejsoEchartsInfo kejsoEchartsInfo = superSearchService.searchRelatedField(keyword);
                if (kejsoEchartsInfo != null && kejsoEchartsInfo.getNodes() != null) {
                    interestArr.clear();
                    for (KejsoEchartsNodesInfo nodesInfo : kejsoEchartsInfo.getNodes()) {
                        interestArr.add(nodesInfo.getName());
                    }
                }
            }
        }
    ...
    public String search(HttpServletRequest request, Model model, @PathVariable Long mapId) {
        logger.debug("invoke---/front/instrument/search/" + mapId);
...
//                result = "{\"address\":\"CN|\\u6e56\\u5317|\\u6b66\\u6c49|None|CHINANET|0|0\",\"content\":{\"address\":\"\\u6e56\\u5317\\u7701\\u6b66\\u6c49\\u5e02\",\"address_detail\":{\"city\":\"\\u6b66\\u6c49\\u5e02\",\"city_code\":218,\"district\":\"\",\"province\":\"\\u6e56\\u5317\\u7701\",\"street\":\"\",\"street_number\":\"\"},\"point\":{\"x\":\"114.31620010\",\"y\":\"30.58108413\"}},\"status\":0}";
                ...
                if (status != null && status == 0) {
                    ...
                    request.getSession().setAttribute("lng", latlng.get("x"));
                    request.getSession().setAttribute("lat", latlng.get("y"));
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
