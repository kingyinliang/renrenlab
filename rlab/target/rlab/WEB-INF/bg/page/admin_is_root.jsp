...
<div class="wrapper">
    ...
    <div class="comn">
        ...
        <div class="main" style="overflow-y: auto">
            ...
            <%--内容部分--%>
                <%--111 <br>--%>
                <%--${sessionScope.u_permisssion_array[0]}--%>
                <%--${sessionScope.u_permission}--%>
                <%--${sessionScope.u_permission.length}--%>
                <%--${sessionScope.u_r_name}--%>
                <%--${sessionScope.u_permisssion_array.size()}--%>
                <div class="admin">
                            ...
                            <div class="ipt_min" style="">
                                <select id="changeAdmin" lay-filter="changeAdmin">
                                    ...
                                    <c:if test="${'超级管理员'.equals(sessionScope.u_r_name)}">
                                        <option value="10008" selected>高级管理员</option>
                                    </c:if>
                                    <option value="10006" ${'高级管理员'.equals(sessionScope.u_r_name)?'selected':''}>中级管理员</option>
                                    <option value="10005">初级管理员</option>
                                </select>
                            </div>
                        </label>
    ...
    layui.use(['form', 'layer'], function () {
        ...
        form.on('select(changeAdmin)', function (data) {
            // 1.判断当前页面是否修改
                ...
                if(rName == "高级管理员"){
                    $("#p3").show(400,function () {
                        $("#p3").css("overflow","visible");
                    })
                    $("#p4").show(400,function () {
                        $("#p4").css("overflow","visible");
                    });
                ...
                getLimitByGrade(rName, function (data) {
                    ...
                    var rPermisssion = dealPermission(data.rPermisssion)
                    for (var i = 0; i < data.length; i++) {
                        console.log(data);
                        console.log(ipt.eq(data[i]));
                        ipt.eq(data[i]).prop("checked", true);
                        ipt.eq(data[i]).parent().removeClass("cancel").addClass("checked");
                    }

                    if(rPermisssion.p1 == 1){
                       $("#p1 input").prop("checked", true);
                       $("#p1 input").parent().removeClass("cancel").addClass("checked");
                    }
                    if(rPermisssion.p2 == 1){
                        $("#p2 input").prop("checked", true);
                        $("#p2 input").parent().removeClass("cancel").addClass("checked");
                    }
                    if(rPermisssion.p3 == 1){
                        $("#p3 input").prop("checked", true);
                        $("#p3 input").parent().removeClass("cancel").addClass("checked");
                    }
                    if(rPermisssion.p4 == 1){
...
//            return false;
//        }
        var p4 = $("#p4 input").prop("checked")?"1":"0";
        var p3 = $("#p3 input").prop("checked")?"1":"0";
        var p2 = $("#p2 input").prop("checked")?"1":"0";
        var p1 = $("#p1 input").prop("checked")?"1":"0";

        var param = {
            rRid:$("#changeAdmin").val(),
            rPermisssion:"0000" + p4 + p3 + p2 + p1
        }
    ...
    function alterLimitByPost(param, callback) {
        $.ajax({
            url: '${rlab}/bg/user/modify_permission',
            type: 'POST',
