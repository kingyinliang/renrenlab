                            ...
                            <div class="layui-inline">
                                <div class="layui-input-inline">
                                    <select id="shareInsStatus" lay-filter="shareInsStatus" name="modules"
                                        ...
                                        <option value="3" ${state eq "3"?"selected":""}>备案完成、款项发放中</option>
                                        <option value="4" ${state eq "4"?"selected":""}>项目完成</option>
                                    </select>
                                </div>
                            </div>
                        </form>
            ...
            <div class="lab_list_type1">
                <table class="layui-table">
                    ...
                    <tr>
                        ...
                        <td>
                            <c:if test="${info.orgProState eq 0}">
                                待审核
                            </c:if>
                            <c:if test="${info.orgProState eq 1}">
                                审核通过
                            <c:if test="${not empty info.orgProContract}">
                                <br>
                                <span style="font-size: 12px;color: #fb6a5f;">已经上传备案材料</span>
                            </c:if>
                            </c:if>
                            <c:if test="${info.orgProState eq 2}">
                                审核拒绝
                            </c:if>
                            <c:if test="${info.orgProState eq 3}">
                                备案完成、款项发放中
                            </c:if>
                            <c:if test="${info.orgProState eq 4}">
                                项目完成
                            </c:if>
                        </td>
                        <td>
...
//                SHARE_INS_STATUS = data.value;
            ...
            form.on('select(pageSize)', function (data) {
                pageSize = data.value;
                window.location.href = '${rlab}/bg/coupon/search?flag=2&keyword=' + keyword + '&pageNo=1&pageSize=' + pageSize + '&state=' + mapState + '&startTime=' + startDate + '&endTime=' + endDate+order;
            });
            ...
            var pagesOption = {
                ...
                , jump: function (obj, first) {
                    ...
                    if (!first) {
                        window.location.href = '${rlab}/bg/coupon/search?flag=2&keyword=' + keyword + '&pageNo=' + obj.curr + '&pageSize=' + pageSize + '&state=' + mapState + '&startTime=' + startDate + '&endTime=' + endDate+order;
                    }
                }
            };
            // 挂载分页组件
    ...
    function doSearchSharew(scRange) {
        ...
        if(scRange=='all'){
            window.location.href ='${rlab}/bg/coupon/search?flag=2&startTime=2016-01-01';
            return;
        }
        var url = '${rlab}/bg/coupon/search?flag=2&keyword=' + keyword + '&pageNo=1&pageSize=' + pageSize + '&state=' + mapState + '&startTime=' + startDate + '&endTime=' + endDate+order;
