...
function showPages(total, from, max, callback, ContainerId) {
    ...
    $prev = $("<button class='items' type='button'><span class='lab-arrow-left'></span></button>");
    ...
    $next = $("<button class='items'><span class='lab-arrow-right'></span></button>");
    if (current == count) {
    ...
    $topage = $('<li style="display: none;" class="page_seach" style="padding-left:5px">共' + count + '页，到第<input type="text">页 <button class="btn btn-blue">确定</button></li>');
...
// 分页样式2
function showPagesSpe(total, from, max, callback, ContainerId) {
    ...
    //上一页
    $prev = $('<li><a><span class="lab-arrow-left"></span></a></li>');
    if (current == 1) {
        $prev.addClass('prev disabled');
    } else {
        $prev.removeClass('prev disabled');
    ...
    if (count < blocksize + 1) {
        ...
        } else if (current > count - 3 && current > 3) {
            for (var i = count - blocksize + 1; i < count + 1; i++) {
                $page = $("<li><a>" + i + "</a></li>");
                $page.click(function () {
                    var pagenum = $(this).text();
    ...
    //下一页
    $next = $('<li><a><span class="lab-arrow-right"></span></a></li>');
    if (current == count) {
        $next.addClass('next disabled');
    } else {
        $next.removeClass('prev disabled');
    ...
    $next.appendTo('#' + ContainerId);
    $topage = $('<li style="padding-left:5px">共' + count + '页，到第<input type="text">页 <button class="btn btn-blue">确定</button></li>');
    $topage.find('button').click(function (event) {
        var aimPage = $topage.find('input').val();
        if (aimPage != "" && aimPage <= count && aimPage > 0) {
            pageClick(max, aimPage, callback);
